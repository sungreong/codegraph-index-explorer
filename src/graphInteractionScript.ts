export function getGraphInteractionScript(): string {
  return `
    ...
    function showDetails(node, neighborPage) {
      ...
      els.details.innerHTML = '<div class="detail-head ' + escapeHtml(node.type) + '"><div class="node-orb"></div><div><h2>' + escapeHtml(node.label) + '</h2><div class="detail-kind">' + escapeHtml(node.type) + '</div></div></div>' +
        ...
        (canStepDown ? '<button class="icon-button ghost" id="showLess" type="button" title="Show fewer nodes" aria-label="Show fewer nodes">−</button>' : '') +
        (raw.omitted ? '<button class="icon-button ghost" id="showMore" type="button" title="' + escapeHtml(showMoreLabel(raw)) + '" aria-label="' + escapeHtml(showMoreLabel(raw)) + '" ' + (canStepLimit ? '' : 'disabled') + '>＋</button><button class="icon-button ghost" id="showAll" type="button" title="Render all hidden items" aria-label="Render all hidden items">All</button>' : '') +
        ...
        (canOpenNode(node) ? '<button class="icon-button ghost" id="openSource" type="button" title="Open source" aria-label="Open source">↗</button>' : '') +
        (expandable ? '<button class="icon-button ghost" type="button" data-expand-mode="callers" title="Add callers" aria-label="Add callers">In</button><button class="icon-button ghost" type="button" data-expand-mode="callees" title="Add callees" aria-label="Add callees">Out</button><button class="icon-button ghost" type="button" data-expand-mode="impact" title="Add impact" aria-label="Add impact">Fx</button><button class="icon-button ghost" type="button" data-expand-mode="all" title="Add callers, callees, and impact" aria-label="Add all relationships">All</button>' : '') +
        (renderedNode ? '<button class="icon-button ghost" id="centerNode" type="button" title="Center selected node" aria-label="Center selected node">⌖</button><button class="icon-button ghost" id="stabilizeNode" type="button" title="Stabilize graph layout" aria-label="Stabilize graph layout">↺</button>' : '') + '</div>' +
